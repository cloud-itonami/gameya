(ns gameya.app
  "gameya-play-canvas appview — reagent + re-frame, view built from
  ...
  The Svelte page never called the dispatcher or the actual game logic in
  `../src/app.ts` — it was pure static markup describing the surface's own
  metadata, and this port adds no game functionality that was not already
  there. Per this repo's README, this scaffold is NOT the game
  (`../src/app.ts`, a self-contained Canvas Worker, is) and was NOT part of
  the deployable before this migration either — `wrangler.jsonc` built this
  scaffold, not the game. That split is unchanged by this port; only the
  scaffold's implementation language moved.

  The Svelte scaffold's `routes`/`vars` arrays were stale literals
  (`routeCount: 0`, empty arrays) even though `wrangler.jsonc` already
  declared 2 routes and 8 vars; this port corrects them from the
  component's actual `wrangler.jsonc` (`routes[].pattern` and the `vars`
  map keys), same as the landed reference this was copied from
  (`cloud-itonami/app-tia`'s `tia-mcp-component/cljs`).

  Backend TypeScript is unaffected by this migration: `../src/app.ts` (the
  game Worker) is untouched. The one other TypeScript file that lived under
  the deleted `svelte/` tree — `svelte/src/routes/xrpc/[...path]/+server.ts`,
  a SvelteKit server route that proxied XRPC calls to
...
;; svelte/src/routes/+page.svelte to here. :route-count/:routes/:vars are
;; corrected from wrangler.jsonc (the Svelte scaffold had stale
;; zero/empty values there).
...
(def default-db
  {:title "Gameya Play Canvas"
   ...
   :kind "appview"
   :route-count 2
   :routes ["g4m3ya00.etzhayyim.com/*" "gameya.etzhayyim.com/*"]
   :vars ["AGENTGATEWAY_MCP_ROUTER_URL" "APP_CAPABILITIES" "APP_DESCRIPTION"
          ...
          "APP_UI_TYPE"]
   :xrpc? true
   :relative-path "appview/gameya-play-canvas/cljs/src/gameya/app.cljs"})
