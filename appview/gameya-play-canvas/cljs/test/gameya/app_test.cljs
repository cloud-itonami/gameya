...
(deftest initialize-db-sets-defaults
  (testing ":initialize-db populates the ported +page.svelte `app` object"
    ...
    (is (= "Gameya Play Canvas" @(rf/subscribe [:title])))
    (is (= "etzhayyim-project-gameya" @(rf/subscribe [:project])))
    (is (= "gameya-play-canvas" @(rf/subscribe [:name])))
    (is (= "appview" @(rf/subscribe [:kind])))
    (is (= 2 @(rf/subscribe [:route-count])))
    (is (= ["g4m3ya00.etzhayyim.com/*" "gameya.etzhayyim.com/*"]
           @(rf/subscribe [:routes])))
    (is (= 8 (count @(rf/subscribe [:vars]))))
    (is (true? @(rf/subscribe [:xrpc?])))
    (is (= "appview/gameya-play-canvas/cljs/src/gameya/app.cljs"
